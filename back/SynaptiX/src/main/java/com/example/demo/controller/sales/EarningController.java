...
public class EarningController {
    ...
    public void downloadEarningExcelModal(
        ...
        try {
            ...
            for (Map<String, Object> s : earningList) {
                writer.println(
                    (s.get("ProductId") != null ? s.get("ProductId").toString() : "") + "," +
                    (s.get("Date") != null ? s.get("Date").toString() : "") + "," +
                    (s.get("ProductName") != null ? s.get("ProductName").toString() : "") + "," +
                    (s.get("Amount") != null ? s.get("Amount").toString() : "0") + "," +
                    (s.get("Price") != null ? s.get("Price").toString() : "0") + "," +
                    (s.get("Total") != null ? s.get("Total").toString() : "0") + "," +
                    (s.get("Earning") != null ? s.get("Earning").toString() : "0") + "," +
                    (s.get("Stock") != null ? s.get("Stock").toString() : "0")
                );
            }
            writer.flush();
            writer.close();
            ...
            response.getWriter().write("CSV 다운로드 중 서버 오류: " + e.getMessage());
        }
    }
}